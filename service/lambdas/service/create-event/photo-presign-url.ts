...
const handleSingleFile = async (
  ...
  try {
    ...
    console.log('error in', { eventId, organization });
    ...
    if (result.err) console.log(result.reason, { eventId, organization });
  }
  if (result.success) {
    console.log('start presign url request', { eventId, organization, fileName, fileSize });
...
const photoPresignLambda = async (event: APIGatewayProxyEvent) => {
  ...
  console.log('start presign url request', { eventId, organization });
  if (!files || !eventId) {
